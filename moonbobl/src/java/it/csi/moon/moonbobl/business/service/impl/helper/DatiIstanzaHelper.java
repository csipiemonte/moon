...
public class DatiIstanzaHelper {
	...
	protected JsonNode dataNode = null;
	...
	public String extractedTextValueFromDataNodeByKey(String fullKey) {
		...
		return dataTextNode!=null?dataTextNode.getTextValue():null;
	}
	public String extractedTextValueFromDataNodeByKey(String fullKey, String formioType) {
		...
		if ("Number".equalsIgnoreCase(formioType))
			return dataTextNode.getNumberValue().toString();
		else if ("Boolean".equalsIgnoreCase(formioType))
			return dataTextNode.getBooleanValue()?"S":"N";
		else
			return dataTextNode.getTextValue();
	}
	public boolean extractedBooleanValueFromDataNodeByKey(String fullKey) {
		...
		return dataBooleanNode!=null?dataBooleanNode.getBooleanValue():false;
	}
	...
	public JsonNode extractedJsonNodeFromDataNodeByKey(String fullKey) {
		...
		if (fullKey.contains(".")) {
			...
			log.debug("[" + CLASS_NAME + "::extractedJsonNodeFromDataNodeByKey] fullKey.split('.') : " + keysArray + " of length " + keysArray.length);
			int i = 0;
			while (i<(keysArray.length-1)) {
				log.debug("[" + CLASS_NAME + "::extractedJsonNodeFromDataNodeByKey] ricerca data node " + i + " - " + keysArray[i] + " of fullKey: " + fullKey);
		...
		if (data != null && data.get(key) != null) {
			if (data.get(key).isArray()) {
				log.warn("[" + CLASS_NAME + "::extractedJsonNodeFromDataNodeByKey] Key: " + key + " of fullKey: " + fullKey + " is not Object but Array in istanza dataNode\n" + dataNode);
			} else {
				log.debug("[" + CLASS_NAME + "::extractedJsonNodeFromDataNodeByKey] data=" + data);
				try {
					return data.has(key)?data.get(key):null;
					// Magari dopo gestire altre tipologie di campo oltre che String : Magari gestire Integer, ...
				} catch (Exception e) {
					log.error("[" + CLASS_NAME + "::extractedJsonNodeFromDataNodeByKey] Key: " + key + " of fullKey: " + fullKey + " is not String quindi non usare .getTextValue()\n" + dataNode);
				}
			}
		} else {
			log.warn("[" + CLASS_NAME + "::extractedJsonNodeFromDataNodeByKey] Key: " + key + " of fullKey: " + fullKey + " not found in ["+data+"] del istanza dataNode\n" + dataNode);
		}
		return null;
	}
	...
	public List<String> extractFormIoFileNameByCampo(CampoModulo campo) {
		...
		if (!StrUtils.isEmpty(campo.getGridFullKey())) {
			...
			if (gridFullKey.contains(".")) {
				...
				log.debug("[" + CLASS_NAME + "::extractFormIoFileNameByCampo] gridFullKey.split('.') : " + gridKeysArray + " of length " + gridKeysArray.length);
				int i = 0;
...
//						if (localData.get(campo.getKey()) != null) {
//							result = extractedFormIoFileNameTextValueByCampo(campo, result, localData);
//						} else {
//							log.debug("[" + CLASS_NAME + "::extractedFormIoFileNameTextValueFromDataNodeByCampo] GRID("+i+") null or "+campo.getKey()+" not found (magari campo opzionale!) "+data.get(gridKey).get(i));
//						}
					}
				}
			} else {
	...
	private List<String> _extractFormIoFileNameByCampo(CampoModulo campo, List<String> result, JsonNode data) {
		...
		if (fullKey.contains(".")) {
			String[] keysArray = fullKey.split("\\.");
			log.debug("[" + CLASS_NAME + "::_extractFormIoFileNameByCampo] fullKey.split('.') : " + keysArray + " of length " + keysArray.length);
			int i = 0;
			while (i<(keysArray.length-1)) {
				log.debug("[" + CLASS_NAME + "::_extractFormIoFileNameByCampo] ricerca data node " + i + " - " + keysArray[i] + " of fullKey: " + fullKey);
				data = data.get(keysArray[i]);
				if (data == null) {
			...
			if (data.get(key).isArray()) { // Il componente FileUpload e' sempre un array
				for (int i = 0; i < data.get(key).size(); i++) {
					if (data.get(key).get(i) != null && data.get(key).get(i).get("name") != null) {
						result.add(data.get(key).get(i).get("name").getTextValue());
					} else {
						log.warn("[" + CLASS_NAME + "::_extractFormIoFileNameByCampo] Key: " + key+ " of fullKey: " + fullKey
								+ " impossibile estrarre get(0).get('name') nel istanza per dataNode.get(key)=\n"
								+ dataNode.get(key));
					}
				}
			} else {
				log.warn("[" + CLASS_NAME + "::_extractFormIoFileNameByCampo] Key: " + key + " of fullKey: " + fullKey + " is not Array nel istanza dataNode\n" + dataNode);
			}
		} else {
			log.warn("[" + CLASS_NAME + "::_extractFormIoFileNameByCampo] Key: " + key + " of fullKey: " + fullKey + " not found in ["+data+"] del istanza dataNode\n" + dataNode);
		}
		return result;
	}
	...
	public List<AllegatoLazyEntity> verificaMatchFullKeyAllegati(List<AllegatoLazyEntity> allegati, List<CampoModulo> campiFile) {
		...
		try {
			...
			result = new ArrayList<AllegatoLazyEntity>();
