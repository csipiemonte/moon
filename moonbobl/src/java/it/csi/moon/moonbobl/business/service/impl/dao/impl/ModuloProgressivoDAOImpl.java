...
public class ModuloProgressivoDAOImpl extends JdbcTemplateDAO implements ModuloProgressivoDAO {
	private final static String CLASS_NAME = "ModuloProgressivoDAOImpl";
	...
	public ModuloProgressivoEntity findByIdModuloForUpdate(Long idModulo) throws DAOException {
		try {
			log.debug("[" + CLASS_NAME + "::findByIdModuloForUpdate] IN idModulo: "+idModulo);
			MapSqlParameterSource params = new MapSqlParameterSource();
			params.addValue("id_modulo", idModulo);
			return (ModuloProgressivoEntity)getCustomNamedParameterJdbcTemplateImpl().queryForObject(FIND + " FOR UPDATE", params, BeanPropertyRowMapper.newInstance(ModuloProgressivoEntity.class)  );
		} catch (EmptyResultDataAccessException emptyEx) {
...
//	@Override
	private ModuloProgressivoEntity findByModuloForUpdate2(ModuloEntity modulo) throws DAOException {
		try {
	...
	private String getProgressivo(ModuloEntity moduloE, Boolean isAnnuale, Integer year) throws DAOException {
		...
		try {
			progressivoE = findByModuloForUpdate2(moduloE);
