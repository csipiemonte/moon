...
public class ProcessoModuloDAOImpl extends JdbcTemplateDAO implements ProcessoModuloDAO {
	...
	private final static String CLASS_NAME = "ProcessoModuloDAOImpl";
	...
	private static final String UPDATE_PROCESSO = "UPDATE moon_wf_r_modulo_processo SET id_processo=:id_processo WHERE id_modulo=:id_modulo AND ordine=:ordine";
	...
	@Override
	public int updateProcesso(ProcessoModuloEntity entity) throws DAOException {
		try {
			log.debug("[" + CLASS_NAME + "::updateProcesso] IN entity: "+entity);		
			int numRecord = getCustomNamedParameterJdbcTemplateImpl().update(UPDATE_PROCESSO, mapProcessoModuloEntityParameters(entity));
			log.debug("[" + CLASS_NAME + "::updateProcesso] Record inseriti: " + numRecord);
			return numRecord;
		} catch (Exception e) {
			log.error("[" + CLASS_NAME + "::updateProcesso] Errore database: "+e.getMessage(),e);
			throw new DAOException("ERRORE UPDATE_PROCESSO");
		}
	}
