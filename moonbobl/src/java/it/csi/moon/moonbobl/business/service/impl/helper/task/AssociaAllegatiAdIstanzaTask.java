...
public class AssociaAllegatiAdIstanzaTask implements Callable<String> {
	...
	private final static String CLASS_NAME = "AssociaAllegatiAdIstanzaTask";
...
//		this.threadName = Thread.currentThread().getName();
	...
	public String call() throws BusinessException {
		try {
			...
			try {
				List<CampoModulo> campiFile = moonsrvDAO.getCampiModulo(istanzaSaved.getModulo().getIdModulo(),istanzaSaved.getModulo().getIdVersioneModulo(),"file",null);
				if (campiFile == null || campiFile.isEmpty()) {
					...
					List<CampoModuloFormioFileName> campiFormIoFileNames = retriveFormIoFileNames(campiFile);
	...
	private List<CampoModuloFormioFileName> retriveFormIoFileNames(List<CampoModulo> campiFile) throws BusinessException, Exception {
		List<CampoModuloFormioFileName> result = new ArrayList<CampoModuloFormioFileName>();
		for (CampoModulo campo : campiFile) {
			//log.debug("[" + CLASS_NAME + "::retriveFormIoFileNames] campo " + campo);
