...
public enum DecodificaAzione implements DecodificaMOON {
	...
	GESTISCI_PAGAMENTO(28L,"GESTISCI_PAGAMENTO","Avvia pagamento","Avvia il pagamento"),
	...
	/** 61	Paga sportello */
	PAGA_SPORTELLO(61L,"PAGA_SPORTELLO","Stampa avviso di pagamento","Stampa il codice IUV per pagare allo sportello"),
	/** 62	Salva protocollo */
	...
	RESTITUISCI_AL_PROPONENTE(68L, "RESTITUISCI_AL_PROPONENTE", "Restituisci al proponente", "Restituisci al proponente"),
	/** 69 Valida */
	 ...
	 */
	public boolean isCorrectAzione(Long idAzioneToCompare) {
		if (idAzioneToCompare==null) return false;
		if (getIdAzione().equals(idAzioneToCompare))
			return true;
		return false;
	}
	public boolean isCorrectAzione(String codiceAzioneToCompare) {
		if (codiceAzioneToCompare==null) return false;
		if (getCodice().equals(codiceAzioneToCompare))
			return true;
		return false;
	}
	public boolean isCorrectAzione(WorkflowEntity wfE) {
		if (wfE==null) return false;
		if (wfE.getCodiceAzione()!=null && getCodice().equals(wfE.getCodiceAzione()))
			return true;
		if (wfE.getIdAzione()!=null && getIdAzione().equals(wfE.getIdAzione()))
			return true;
		return false;
	}
}
