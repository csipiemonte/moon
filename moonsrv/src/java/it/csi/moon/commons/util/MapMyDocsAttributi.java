...
/**
 * Mappa degli attributi di configurazione per la pubblicazione su MyDocs (DOCME)
 * 
 ...
 *
 */
public class MapMyDocsAttributi {

	public static Map<String, Object> EMPTY_MAP = new HashMap<>();
	private Map<String, Object> attributiMap = new HashMap<>();

	public MapMyDocsAttributi() {
		super();
	}
	public MapMyDocsAttributi(List<MyDocsParametroEntity> listAttributi) {
		super();
		if (listAttributi != null && !listAttributi.isEmpty()) {	
			listAttributi.forEach( mae -> {
				MyDocsAttributoKeys key = MyDocsAttributoKeys.byName(mae.getNomeAttributo());
