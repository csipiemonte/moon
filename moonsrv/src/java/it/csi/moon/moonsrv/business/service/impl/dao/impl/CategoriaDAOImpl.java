...
public class CategoriaDAOImpl extends JdbcTemplateDAO implements CategoriaDAO {
	...
	private static final String CLASS_NAME = "CategoriaDAOImpl";
	...
	private static final String FIND_BY_NOME = "SELECT id_categoria, nome_categoria, id_categoria_padre, data_upd, attore_upd from moon_fo_d_categoria where nome_categoria = :nome_categoria";
	...
	public CategoriaEntity findByNome(String nomeCategoria) throws DAOException {
		try {
			MapSqlParameterSource params = new MapSqlParameterSource();
			params.addValue("nome_categoria", nomeCategoria);
			return (CategoriaEntity)getCustomNamedParameterJdbcTemplateImpl().queryForObject(FIND_BY_NOME, params, BeanPropertyRowMapper.newInstance(CategoriaEntity.class)  );
		} catch (EmptyResultDataAccessException emptyEx) {
	...
	public int deleteCategoriaByIdModulo(Long idModulo) throws DAOException {
		try {
			LOG.debug("[" + CLASS_NAME + "::deleteCategoriaByIdModulo] IN idModulo: "+idModulo);
			MapSqlParameterSource params = new MapSqlParameterSource();
			params.addValue("id_modulo", idModulo);
			int numRecord = getCustomNamedParameterJdbcTemplateImpl().update(DELETE_CATEGORIA_MODULO_BY_ID_MODULO, params);
			LOG.debug("[" + CLASS_NAME + "::deleteCategoriaByIdModulo] Record inseriti: " + numRecord);
			return numRecord;
		} catch (Exception e) {
			LOG.error("[" + CLASS_NAME + "::deleteCategoriaByIdModulo] Errore database: "+e.getMessage(),e);
			throw new DAOException("ERRORE INSERIMENTO");
		}
	}
