...
public class ReportMapper_TARI_UD_CESS extends BaseReportMapper_TARI implements ReportMapper {
	
	private final static String CLASS_NAME = "ReportMapper_TARI_UD_CESS";
	
	public ReportMapper_TARI_UD_CESS(String codiceEstrazione) {
	...
	public String remapIstanza(IstanzaReport istanza) throws Exception {
		...
		//Tipologia Utenza
		String tipologiaUtenza = EXPORT_VAL.UTENZA_DOMESTICA;
		row.append(tipologiaUtenza).append(CSV_SEPARATOR);
		
		//Tipologia Richiesta
		String tipologiaRichiesta = EXPORT_VAL.TIP_RICHIESTA_CESSAZIONE;
		row.append(tipologiaRichiesta).append(CSV_SEPARATOR);
		
		//Codice Utenza
//		String codiceUtenza = getCodiceUtenzaDomestica();
		String codiceUtenza = getCodiceUtenza(istanza, EXPORT_VAL.UTENZA_DOMESTICA);
