...
//	private UserInfo user; // Attenzione nel caso del BO il user è l'operatore !
    ...
    public SendEmailDichiaranteIstanzaTask(Istanza istanza, String confSendEmail) {
    	new SendEmailDichiaranteIstanzaTask(istanza, confSendEmail, "", false);
	}
    
    public SendEmailDichiaranteIstanzaTask(Istanza istanza, String confSendEmail, String dest, boolean rinvio) {
		super();
		this.istanza = istanza;
		this.confSendEmail = confSendEmail;
		this.dest = StringUtils.isBlank(dest)?"user":dest; // Default value user
		this.rinvio = rinvio;
		LOG.debug("[" + CLASS_NAME + "::SendEmailDichiaranteIstanzaTask] CONSTRUCTOR istanzaSaved.getIdIstanza(): " + getIstanza().getIdIstanza());
    ...
    public SendEmailDichiaranteIstanzaTask(Istanza istanza, String confSendEmail, String dest) {
		super();
		this.istanza = istanza;
		this.confSendEmail = confSendEmail;
		this.dest = StringUtils.isBlank(dest)?"user":dest; // Default value user
		this.rinvio = false;
		this.idTipologiaLogEmail = DecodificaTipoLogEmail.SRV_PRT_IN_CALLBACK.getId();
		LOG.debug("[" + CLASS_NAME + "::SendEmailDichiaranteIstanzaTask] CONSTRUCTOR PCPT istanzaSaved.getIdIstanza(): " + getIstanza().getIdIstanza());
    ...
    private void makeValidaSendEmail(String emailUserDichiarante) throws BusinessException, DAOException {
		...
		LOG.debug("[" + CLASS_NAME + "::makeValidaSendEmail] emailService.sendEmail() Ended.");
	}
	...
	private String retrieveSubject() {
		...
		String result = replaceDinamici((rinvio && conf.get("subject_rinvio")!=null)?
			conf.get("subject_rinvio").asText():
			(conf.get("subject")!=null?conf.get("subject").asText():null));
	...
	private String retrieveText() {
		...
		String result = replaceDinamici((rinvio && conf.get("text_rinvio")!=null)?
			conf.get("text_rinvio").asText():
			(conf.get("text")!=null?conf.get("text").asText():null));
	...
	private String retrieveContentHtml() {
		...
		String result = replaceDinamici((rinvio && conf.get("html_rinvio")!=null)?
			conf.get("html_rinvio").asText():
			(conf.get("html")!=null?conf.get("html").asText():null));
	...
	private EmailAttachment retrieveEmailAttach() {
		...
		EmailAttachment result = conf.get("attach_istanza").asBoolean()?new EmailAttachment(getIstanza().getIdIstanza(),retrieveAttachAllegati()):null;
	...
	private Boolean hasSuperamentoLimitAllegati() {
		...
		boolean result = false;
		AllegatiSummary allegatiSummary = allegatiService.getAllegatiSummary(getIstanza().getIdIstanza());
	...
	private String replaceDinamici(String textValue) throws ItemNotFoundDAOException, DAOException {
		...
		if (result.contains("@@")) {
			// User
			result = result.replace("@@IDENTIFICATIVO_UTENTE@@", nullToEmpty(getIstanza().getIdentificativoUtente()));
			result = result.replace("@@CODICE_FISCALE@@", nullToEmpty(getIstanza().getCodiceFiscaleDichiarante())); // Attento non funziona per conto terzi
			result = result.replace("@@Cognome@@", WordUtils.capitalize(nullToEmpty(getIstanza().getCognomeDichiarante())));
			result = result.replace("@@Nome@@", WordUtils.capitalize(nullToEmpty(getIstanza().getNomeDichiarante())));
			result = result.replace("@@COGNOME@@", nullToEmptyUC(getIstanza().getCognomeDichiarante()));
			result = result.replace("@@NOME@@", nullToEmptyUC(getIstanza().getNomeDichiarante()));
			result = result.replace("@@NOME_COGNOME@@", String.join(" ", 
					nullToEmptyUC(getIstanza().getNomeDichiarante()),
					nullToEmptyUC(getIstanza().getCognomeDichiarante())));
			...
			result = result.replace("@@COGNOME-NOME@@", String.join("-", 
					...
					nullToEmptyUC(getIstanza().getNomeDichiarante())));
			//
			result = result.replace("@@ATTORE_INS@@", nullToEmptyUC(getIstanza().getAttoreIns()));
...
//			result = result.replace("@@ID_MODULO_VERSIONE@@", istanzaSaved.getModulo().getIdModuloVersione().toString());
			...
			result = result.replace("@@CREATED@@", nullToEmptySDF(getIstanza().getCreated()));
			result = result.replace("@@MODIFIED@@", nullToEmptySDF(getIstanza().getModified()));
			// Save Istanza Descrizione
//			result = result.replace("@@SAVE_ISTANZA_RESPONSE_DESCRIZIONE@@", istanza!=null?istanza.getDescrizione():null);
			// Specific
			result = replaceAllSpecific(result);
			// Protocollo
			result = result.replace("@@NUMERO_PROTOCOLLO@@", nullToEmpty(getIstanza().getNumeroProtocollo()));
			result = result.replace("@@DATA_PROTOCOLLO@@", nullToEmptySDF(getIstanza().getDataProtocollo()));
		}
