...
public class AreaDAOImpl extends JdbcTemplateDAO implements AreaDAO {
	...
	private static final String CLASS_NAME = "AreaDAOImpl";
	
	private static final String ID_AREA = "id_area";
	private static final String CODICE_AREA = "codice_area";
	private static final String NOME_AREA = "nome_area";
	private static final String ID_ENTE = "id_ente";

	private static final String FIND_SELECT_FIELDS = "SELECT id_area, id_ente, codice_area, nome_area, data_upd, attore_upd";
	...
	public List<AreaEntity> find(AreeFilter filter) throws DAOException {
		...
		result = (List<AreaEntity>) getCustomNamedParameterJdbcTemplateImpl().query(sb.toString(), params, BeanPropertyRowMapper.newInstance(AreaEntity.class));
		
		return result;
	}
	
	private StringBuilder readFilter(AreeFilter filter, MapSqlParameterSource params) {
		StringBuilder sb = new StringBuilder();
		if (filter!=null) {
			readFilterProperty(filter.getIdArea(), " AND id_area = :id_area", ID_AREA, sb, params);
			readFilterProperty(filter.getCodiceArea(), " AND codice_area = :codice_area ", CODICE_AREA, sb, params);
			readFilterProperty(filter.getNomeArea(), " AND nome_area = :nome_area", NOME_AREA, sb, params);
    ...
    private MapSqlParameterSource mapEntityParameters(AreaEntity entity) {
    	...
    	params.addValue(CODICE_AREA , entity.getCodiceArea(), Types.VARCHAR);
