...
import it.csi.moon.moonsrv.business.service.impl.dao.extra.demografia.AnprStatoEsteroDAO;
...
public class AnprNazioneServiceImpl implements AnprNazioneService {
	...
	public List<Nazione> listaNazioni(String uso, String ue) throws BusinessException {
		...
		//return anprStatoEsteroDAO.find(uso, ue).stream().map(se -> remapSe2Nazione(se)).collect(Collectors.toList());
		
		List<Nazione> nations = anprStatoEsteroDAO.find(uso, ue).stream().map(se -> remapSe2Nazione(se)).collect(Collectors.toList());		
		List<Nazione> subset = nations.stream().filter(e -> e.getNome().startsWith("-")).collect(Collectors.toList());		
		nations.removeAll(subset);		 
		List<Nazione> sorted =  nations.stream()
	      .sorted(Comparator.comparing(Nazione::getNome))
