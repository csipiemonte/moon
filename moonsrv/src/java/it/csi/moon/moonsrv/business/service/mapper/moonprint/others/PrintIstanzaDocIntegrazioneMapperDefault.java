 ...
 * @since 1.0.0
 */
public class PrintIstanzaDocIntegrazioneMapperDefault {

	private static final String CLASS_NAME = "PrintIstanzaDocIntegrazioneMapperDefault";
...
//    			"dataGridLabel", 
	 ...
	 * @throws Exception 
	 */
	public MoonprintDocument remap(StoricoWorkflow storicoWf, DatiAzioneEntity datiAzioneModuloE, IstanzaEntity istanzaE) throws BusinessException {
		try {
			localWriter.clean();
			String prefixTitle = datiAzioneModuloE.getDescrizioneDatiazione();
			if (StrUtils.isEmpty(prefixTitle)) {
				prefixTitle = "Integrazione";
			}
			localWriter.setTitle( prefixTitle + " su istanza " + istanzaE.getCodiceIstanza());
	
			// Metadata
			final DateFormat df = new SimpleDateFormat("dd-MM-yyyy");
			final DateFormat dt = new SimpleDateFormat("HH:mm");
	
//			String dataOra = df.format(storicoWf.getDataFine()) + " alle ore " + dt.format(storicoWf.getDataFine());
			
			String dataOra = "alle ore " + dt.format(storicoWf.getDataInizio());
			localWriter.setMetadataDataPresentazione(dataOra);
			localWriter.setMetadataNumeroIstanza(istanzaE.getCodiceIstanza());
			localWriter.setMetadataQrContent(istanzaE.getCodiceIstanza()); // TODO
			localWriter.setMetadataHeader("%%title%%", "", "Integrato il: %%dataPresentazione%%");
			int year = Calendar.getInstance().get(Calendar.YEAR);
			localWriter.setMetadataFooter("Integrazione Istanza n.: %%numeroIstanza%%-"+storicoWf.getIdStoricoWorkflow(), null, "CSI "+year);
	
			// Lettura delle due strutture variabile
			JsonNode istanzaJsonNode = readDatiAzione(storicoWf);
			JsonNode moduloJsonNode = readStrutturaDatiAzione(datiAzioneModuloE);
			...
			LOG.error("[" + CLASS_NAME + "::remap] generico", e);
	 ...
	 * @throws Exception
	 */
	private static JsonNode readDatiAzione(StoricoWorkflow storicoWorkflow) throws Exception {
		try {
			LOG.debug("[" + CLASS_NAME + "::readDatiAzione] IN storicoWorkflow: "+storicoWorkflow);
			ObjectMapper objectMapper = new ObjectMapper()
					.disable(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES);
			JsonNode result = objectMapper.readValue(storicoWorkflow.getDatiAzione().toString(), JsonNode.class);
			return result;
		} catch (IOException e) {
			LOG.error("[" + CLASS_NAME + "::readDatiAzione] IOException ", e);
			throw e;
		} finally {
			LOG.debug("[" + CLASS_NAME + "::readDatiAzione] END");
		}
	}
	 ...
	 * @throws IOException
	 */
	private static JsonNode readStrutturaDatiAzione(DatiAzioneEntity datiAzioneEntity) throws JsonParseException, JsonMappingException, IOException {
		try {
			LOG.debug("[" + CLASS_NAME + "::readStrutturaDatiAzione] IN datiAzioneEntity: "+datiAzioneEntity);

			ObjectMapper objectMapper = new ObjectMapper();

			JsonNode result = objectMapper.readValue(datiAzioneEntity.getStruttura(), JsonNode.class);

			return result;
		} catch (IOException e) {
			LOG.error("[" + CLASS_NAME + "::readStrutturaDatiAzione] ERROR ", e);
			throw e;
		} finally {
			LOG.debug("[" + CLASS_NAME + "::readStrutturaDatiAzione] END");
		}
	}
