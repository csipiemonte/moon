...
public class EpayRichiestaDAOImpl extends JdbcTemplateDAO implements EpayRichiestaDAO {
	...
	private static final  String FIND_BY_ID_ISTANZA_ORDERED  = FIND_SELECT_FIELDS +
		" FROM moon_ep_t_richiesta" +
		" WHERE id_istanza = :id_istanza" + 
		" ORDER BY id_richiesta ASC"; // L'ultima deve essere la piu recente
	...
	public List<EpayRichiestaEntity> findByIdIstanza(Long idIstanza) {
		try {
			...
			result = (List<EpayRichiestaEntity>) getCustomNamedParameterJdbcTemplateImpl().query(FIND_BY_ID_ISTANZA_ORDERED, params, BeanPropertyRowMapper.newInstance(EpayRichiestaEntity.class));
			return result;
		} catch (Exception e) {
