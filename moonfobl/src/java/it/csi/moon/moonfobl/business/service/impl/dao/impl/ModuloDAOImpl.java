...
import it.csi.moon.moonfobl.dto.moonfobl.VersioneStatoEnum;
...
public class ModuloDAOImpl extends JdbcTemplateDAO implements ModuloDAO {
	...
	private static final String LAST_STATO = " and cron.data_fine_validita is NULL";
	...
	public List<ModuloVersioneStato> findVersioniModuloById(Long idModulo, String stato) {
		...
		stato = (stato == null) ? VersioneStatoEnum.ALL.name(): stato;
		
        switch (VersioneStatoEnum.byName(stato)) {
        case ALL:          
            break;
        case CURRENT:
        	params.addValue(NOW, new Timestamp(System.currentTimeMillis()), Types.TIMESTAMP);
        	sbSingleQuery.append(CURRENT_STATO);
            break;
        case LAST:
        	sbSingleQuery.append(LAST_STATO);
            break;
        default:
        	LOG.error("[" + CLASS_NAME + "::findVersioniModuloById] stato  : " + stato);
       
        }		
		sbSingleQuery.append(FIND_VERSIONI_STATI_PART2);	
