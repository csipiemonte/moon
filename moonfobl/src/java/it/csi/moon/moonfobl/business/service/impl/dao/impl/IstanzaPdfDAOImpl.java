...
public class IstanzaPdfDAOImpl extends JdbcTemplateDAO implements IstanzaPdfDAO {
	...
	private static final String SELECT_FULL_FILEDS = "SELECT id_pdf_istanza, id_istanza, id_modulo, hash_pdf, contenuto_pdf, resoconto, data_ins, attore_ins, data_upd, attore_upd, uuid_index" + 
	...
	private static final String UPDATE = "UPDATE moon_fo_t_pdf_istanza" + 
			" SET id_istanza=?, id_modulo=?, hash_pdf=?, contenuto_pdf=?, resoconto=?, data_upd=?, attore_upd=?, uuid_index=?" + // data_ins=?, attore_ins=?, 
	...
	private static final String UPDATE_UUID_INDEX = "UPDATE moon_fo_t_pdf_istanza" + 
	...
	public Optional<IstanzaPdfEntity> findByIdIstanza(Long idIstanza) throws ItemNotFoundDAOException, DAOException {
		try {
			...
			LOG.warn("[" + CLASS_NAME + "::findByIdIstanza] IstanzaPdfEntity non trovato for idIstanza:" + idIstanza);
...
//		    		new SqlParameterValue(Types.VARCHAR, entity.getAttoreIns()),
	...
	public int updateUuidIndex(IstanzaPdfEntity entityPdf) {
		try {
			LOG.debug("[" + CLASS_NAME + "::updateUuidIndex] IN allegato: "+entityPdf.getIdPdfIstanza());
			MapSqlParameterSource params = new MapSqlParameterSource();
	    	params.addValue("uuid_index", entityPdf.getUuidIndex());
	    	params.addValue("id_pdf_istanza", entityPdf.getIdPdfIstanza());
