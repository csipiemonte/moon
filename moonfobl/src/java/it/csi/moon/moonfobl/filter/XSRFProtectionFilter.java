...
public class XSRFProtectionFilter implements Filter {
	...
	static final Logger LOG = Logger.getLogger(Constants.COMPONENT_NAME + ".security");
	...
	/*
	 * Path applicativi in cui viene eseguito un controllo light per XSRF
	 * Per questi path non viene confrontato il token header con quello di sessione
	 */
	
	private static final Set<String> PATHS_LIGHT_CHECK = Collections.unmodifiableSet(new HashSet<>(
            Arrays.asList(
            	"/restfacade/be/extra/",
            	"/restfacade/be/file"
            	)));
	...
	public void doFilter(ServletRequest req, ServletResponse resp, FilterChain chain)
		...
		LOG.debug("[XSRFProtectionFilter::dofilter()] PATH = " + path);
		boolean isLightPath = false; // PATHS_LIGHT_CHECK.contains(path) ;
    	Iterator<String> allowedInitPathItr = PATHS_LIGHT_CHECK.iterator();
    	while(!isLightPath && allowedInitPathItr.hasNext()) { 
			if (!isLightPath && path.startsWith(allowedInitPathItr.next())) {
				isLightPath = true;
			}
    	}
		
		LOG.debug("[XSRFProtectionFilter::dofilter()] isLightPath = " + isLightPath);
		...
		if (!isDisabled) {
			...
			if (!isRestRequest(hreq) || (devmode && isCurrentUser(hreq))) {
				...
				if (!isXSRFSessionAlive(session)) {
					...
					try {
						...
						LOG.debug(
								"[XSRFProtectionFilter::dofilter() - new XSRF token created and added to session and response] tokek: " + newToken);						
				...
				if (isXSRFSessionAlive(session)) {
					...
					if (validXSRFCookieAndHeader(hreq, session, isLightPath)) {
	...
	private boolean isCurrentUser(HttpServletRequest hreq) {
		// TODO Auto-generated method stub
		return hreq.getRequestURI() != null && hreq.getRequestURI().contains("/currentUser");
	}
...
//		c.setHttpOnly(true); // Compliant: this sensitive cookie is protected against theft (HttpOnly=true)
	 ...
	 */
	private boolean validXSRFCookieAndHeader(HttpServletRequest hreq, HttpSession session, boolean isLightPath) {
		String actualActiveToken = getActualActiveXSRFToken(session);
		LOG.debug(
				"[XSRFProtectionFilter::dofilter() - actualActiveToken :  " + actualActiveToken);
		String actualRequestHeader = getActualXSRFHeader(hreq);
		LOG.debug(
				"[XSRFProtectionFilter::dofilter() - actualRequestHeader :  " + actualRequestHeader);
		String actualRequestCookie = getActualXSRFCookie(hreq);
		LOG.debug(
				"[XSRFProtectionFilter::dofilter() - actualRequestCookie :  " + actualRequestCookie);
		if (!isLightPath) {
			...
			LOG.debug(
					"[XSRFProtectionFilter::dofilter() - check light");
			LOG.debug(
					"[XSRFProtectionFilter::dofilter() - Valore ritornato: " + (actualActiveToken != null && actualActiveToken.equals(actualRequestCookie)) );
			    // controllo light per xrf
				
				return actualActiveToken != null && actualActiveToken.equals(actualRequestCookie);
		}
	}
	...
	public void init(FilterConfig cfg) throws ServletException {
		String sDevmode = cfg.getInitParameter(DEVMODE_INIT_PARAM);
